class Urls:
    def __init__(self):
        ...
        self.action_get_auto_switching_and_profits_statistics = 'getautoswitchingandprofitsstatistics'
        ...
        self.get_block_count = self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_block_count)
        self.get_block_stats = self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_block_stats)
        self.get_blocks_found = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_blocks_found)
        self.get_current_workers = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_current_workers)
        self.get_dashboard_data = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_dashboard_data)
        self.get_difficulty = self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_difficulty)
        self.get_estimated_time = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_estimated_time)
        self.get_hourly_hash_rates = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_hourly_hash_rates)
        self.get_nav_bar_data = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_nav_bar_data)
        self.get_pool_hash_rate = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_pool_hash_rate)
        self.get_pool_info = self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_pool_info)
        self.get_pool_share_rate = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_pool_share_rate)
        self.get_pool_status = self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_pool_status)
        self.get_time_since_last_block = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_time_since_last_block)
        self.get_top_contributors = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_top_contributors)
        self.get_user_balance = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_user_balance)
        self.get_user_hash_rate = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_user_hash_rate)
        self.get_user_share_rate = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_user_share_rate)
        self.get_user_status = self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_user_status)
        self.get_user_transactions = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_user_transactions)
        self.get_user_workers = \
            self.protocol + self.coin_pool + self.base_url.format(action=self.action_get_user_workers)
        self.public = self.protocol + self.coin_pool + self.base_url.format(action=self.action_public)

        # Info for all mining pools
        self.get_auto_switching_and_profits_statistics = \
            self.no_pool_base_url.format(action=self.action_get_auto_switching_and_profits_statistics)
        self.get_mining_profit_and_statistics = \
            self.no_pool_base_url.format(action=self.action_get_mining_and_profits_statistics)
        self.get_user_all_balances = self.no_pool_base_url.format(action=self.action_get_user_all_balances)
